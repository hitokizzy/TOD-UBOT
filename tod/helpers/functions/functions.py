...
def higlighted_text(
    ...
    if font_name is None:
        font_name = "tods/helpers/styles/impact.ttf"
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    tod = await sticcers[c_lick].click("me", hide_via=True)
    if tod:
        await borg.send_file(int(chat_id), tod, reply_to=reply_to_id)
        await tod.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    tod = await sticcers[0].click("me", hide_via=True)
    if tod:
        await bot.send_file(int(chat_id), tod, reply_to=reply_to_id)
        await tod.delete()
