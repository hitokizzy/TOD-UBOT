# Heroku manager for your TODUBOT
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        tod = await edit_or_reply(var, "`Getting information...`")
        ...
        tod = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await tod.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await tod.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await tod.edit(
        ...
        tod = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await tod.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await tod.edit(f"`{variable}`**  does not exist**")

        await tod.edit(f"`{variable}`  **successfully deleted**")
