...
async def sticklet(event):
    ...
    image_stream.name = "TODUBOT.webp"
...
async def quby(event):
    ...
    if cmd == "b":
        tod = convert_tosticker(file_name)
        await event.client.send_file(
            event.chat_id, tod, reply_to=reply_to_id, force_document=False
        )
    else:
