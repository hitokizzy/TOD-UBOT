...
async def spam_function(event, zzy, tod, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(tod[0])
    if len(tod) == 2:
        spam_message = str(tod[1])
...
async def spammer(event):
    ...
    await spam_function(event, zzy, tod, sleeptimem, sleeptimet)
...
@tod.tod_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using TODUBOT",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, tod, sleeptimem, sleeptimet, DelaySpam=True)
