...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await tod(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await tod(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
