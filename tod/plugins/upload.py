...
async def sadlst_of_files(path):
    files = []
...
def sortthings(contents, path):
    ...
    for file in contents:
        sadpath = os.path.join(path, file)
        if os.path.isfile(sadpath):
            sadsort.append(file)
    for file in contents:
        sadpath = os.path.join(path, file)
        if os.path.isdir(sadpath):
            sadsort.append(file)
    return sadsort
...
async def upload(path, event, udir_event, sadflag=None):  # sourcery no-metrics
    sadflag = sadflag or False
    ...
    if os.path.isdir(path):
        ...
        for file in Files:
            sadpath = os.path.join(path, file)
            await upload(Path(sadpath), event, udir_event)
...
async def uploadir(event):
    ...
    if os.path.isdir(path):
        ...
        UPLOAD_.uploaded = 0
        await upload(path, event, udir_event, sadflag=flag)
        end = datetime.now()
