...
ported to telethon by @mrconfused and @hitokizzy
"""
...
from ..helpers.utils import _sadtools, _sadutils, reply_id
...
async def glitch(event):
    ...
    if not os.path.isdir("./temp"):
        os.mkdir("./temp")
    sadinput = int(sadinput) if sadinput else 2
    glitch_file = await _sadtools.media_to_pic(event, reply)
    ...
    if cmd:
        ...
        glitch_img = glitcher.glitch_image(img, sadinput, color_offset=True)
        glitch_img.save(glitched)
        await event.client.send_file(event.chat_id, glitched, reply_to=sadid)
    else:
        glitched = os.path.join("./temp", "glitched.gif")
        glitch_img = glitcher.glitch_image(img, sadinput, color_offset=True, gif=True)
        DURATION = 200
        LOOP = 0
        glitch_img[0].save(
            glitched,
        ...
        zzy = await event.client.send_file(event.chat_id, glitched, reply_to=sadid)
