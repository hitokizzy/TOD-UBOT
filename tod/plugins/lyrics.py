# credits to @mrconfused (@hitokizzy)
...
async def lyrics(event):  # sourcery no-metrics
    ...
    if "-" in query:
        ...
        try:
            ...
            songs = None
        if songs is None:
            return await sadevent.edit(f"Song **{artist} - {song}** not found!")
        result = f"**Search query**: \n`{artist} - {song}`\n\n```{songs.lyrics}```"
    else:
        sadevent = await edit_or_reply(event, f"`Searching lyrics for {query}...`")
        response = genius.search_songs(query)
        msg = f"**The songs found for the given query:** `{query}`\n\n"
        if len(response["hits"]) == 0:
            return await edit_or_reply(
                sadevent, f"**I can't find lyrics for the given query: **`{query}`"
            )
    ...
    await edit_or_reply(sadevent, result)
