...
You have {warns}/{totalwarns} warns until you get blocked by the TODUBOT.
    ...
    try:
        if gvarstatus("pmmenu") is None:
            ...
            PM_PIC = (
                ...
                or "https://telegra.ph/file/902492291f182109319ee.png"
            )
            if PM_PIC:
                TOD = [x for x in PM_PIC.split()]
                PIC = list(TOD)
                TOD_IMG = random.choice(PIC)
            else:
                TOD_IMG = None
            if TOD_IMG is not None:
...
@tod.tod_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@tod.tod_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@tod.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@tod.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@tod.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit this options for user who messages you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
...
@tod.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@tod.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "Idoit these options are for users who message you. not for you"
        return await event.answer(text, cache_time=0, alert=True)
