...
async def source(e):
    ...
    await edit_or_reply(
        e,
        "Click [here](https://github.com/hitokizzy/TODUBOT) to open this bot source code"
    )
...
async def _(event):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(URL) as request:
            ...
            sadevent = await edit_or_reply(event, "`fetching github info ...`")
...
@tod.tod_cmd(
    ...
    info={
        ...
        "description": "It uploads the given file to your github repo in **tods/plugins** folder\
...
async def git_commit(file_name, mone):
    ...
    if create_file:
        file_name = "tods/plugins/" + file_name
        LOGS.info(file_name)
        try:
            repo.create_file(
            ...
            ccess = ccess.strip()
            await mone.edit(
                f"`Commited On Your Github Repo`\n\n[Your PLUGINS](https://github.com/{ccess}/tree/master/tods/plugins/)"
            )
