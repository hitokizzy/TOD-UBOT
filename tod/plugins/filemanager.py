...
async def lst(event):  # sourcery no-metrics
    ...
    tod = "".join(event.text.split(maxsplit=1)[1:])
    path = tod or os.getcwd()
    if not os.path.exists(path):
        await edit_or_reply(
            event,
            f"there is no such directory or file with the name `{tod}` check again",
        )
        return
    path = Path(tod) if tod else os.getcwd()
