...
from ..helpers.utils import _format, _sadtools, _sadutils, parse_pre, reply_id
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    media_type(reply)
    sadevent = await edit_or_reply(event, "__Making round spin video wait a sec.....__")
    output = await _sadtools.media_to_pic(event, reply, noedits=True)
    if output[1] is None:
    ...
    if output is None:
        return await edit_delete(sadevent, "__Unable to make spin gif.__")
    ...
    zzy = await event.client.send_file(
        event.chat_id,
        media,
        reply_to=reply,
        video_note=True,
    ...
    if not args:
        await _sadutils.unsavegif(event, zzy)
...
async def video_sadfile(event):  # sourcery no-metrics
    ...
    flag = True
    sadevent = await edit_or_reply(event, "`Converting to round format..........`")
    sadfile = await reply.download_media(file="./temp/")
    if mediatype in ["Gif", "Video", "Sticker"]:
        if not sadfile.endswith((".webp")):
            if sadfile.endswith((".tgs")):
                hmm = await make_gif(sadevent, sadfile)
                os.rename(hmm, "./temp/circle.mp4")
                sadfile = "./temp/circle.mp4"
            media_info = MediaInfo.parse(sadfile)
            aspect_ratio = 1
            for track in media_info.tracks:
                if track.track_type == "Video":
                    aspect_ratio = track.display_aspect_ratio
            ...
            if aspect_ratio != 1:
                crop_by = min(height, width)
                await _sadutils.runcmd(
                    f'ffmpeg -i {sadfile} -vf "crop={crop_by}:{crop_by}" {PATH}'
                )
            else:
                copyfile(sadfile, PATH)
            if str(sadfile) != str(PATH):
                os.remove(sadfile)
            try:
                sadthumb = await reply.download_media(thumb=-1)
            except Exception as e:
        ...
        try:
            ...
            sadthumb = os.path.join("./temp", "thumb.jpg")
            await thumb_from_audio(sadfile, sadthumb)
        if sadthumb is not None and not os.path.exists(sadthumb):
            sadthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, sadthumb)
        if (
            sadthumb is not None
            and not os.path.exists(sadthumb)
            and os.path.exists(thumb_loc)
        ):
            flag = False
            sadthumb = os.path.join("./temp", "thumb.jpg")
            copyfile(thumb_loc, sadthumb)
        if sadthumb is not None and os.path.exists(sadthumb):
            await _sadutils.runcmd(
                f"""ffmpeg -loop 1 -i {sadthumb} -i {sadfile} -c:v libx264 -tune stillimage -c:a aac -b:a 192k -vf \"scale=\'iw-mod (iw,2)\':\'ih-mod(ih,2)\',format=yuv420p\" -shortest -movflags +faststart {PATH}"""
            )
            os.remove(sadfile)
        else:
            os.remove(sadfile)
            return await edit_delete(
                sadevent, "`No thumb found to make it video note`", 5
            )
    if mediatype in [
        "Voice",
        "Audio",
        ...
        if os.path.exists(PATH):
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, sadevent, c_time, "Uploading....")
                ),
            )
            ul.close()
            media = types.InputMediaUploadedDocument(
                ...
                force_file=False,
                thumb=await event.client.upload_file(sadthumb) if sadthumb else None,
            )
            zzy = await event.client.send_file(
                event.chat_id,
                media,
    ...
    img.save(im)
    im.seek(0)
    await event.client.send_file(event.chat_id, im, reply_to=sadid)
    await sadevent.delete()
...
async def _(event):
    ...
    output = await _sadtools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
            output[0], "__Unable to extract image from the replied message.__"
        )
...
async def on_file_to_photo(event):
    ...
    sadt = await edit_or_reply(event, "`Converting.....`")
...
async def _(event):  # sourcery no-metrics
    ...
    if not input_str:
        ...
        if len(loc) == 1:
            ...
            if 0 < loc[0] < 721:
                ...
                return await edit_delete(event, "Use quality of range 0 to 721")
    sadreply = await event.get_reply_message()
    sad_event = "adeevaproject"
    if not sadreply or not sadreply.media or not sadreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if sadreply.media.document.mime_type != "applisadion/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    ...
    try:
        sad_event = Get(sad_event)
        await event.client(sad_event)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    sadfile = await event.client.download_media(sadreply)
    sadgif = await make_gif(event, sadfile, quality, fps)
    zzy = await event.client.send_file(
        event.chat_id,
        sadgif,
...
async def _(event):
    ...
    try:
        ...
        stdout, stderr = await process.communisade()
        stderr.decode().strip()
        stdout.decode().strip()
        os.remove(downloaded_file_name)
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    sadevent = await edit_or_reply(event, "__🎞 Making Gif from the relied media...__")
    imag = await _sadtools.media_to_pic(event, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
    ...
    try:
        ...
        return await edit_delete(sadevent, f"**Error**\n__{e}__")
    output = io.BytesIO()
    output.name = "Output.gif"
    ...
    if output is None:
        await edit_delete(
            sadevent, "__There was some error in the media. I can't format it to gif.__"
        )
        for i in [final, "Output.gif", imag[1]]:
            if os.path.exists(i):
                os.remove(i)
        return
    zzy = await event.client.send_file(event.chat_id, output, reply_to=reply)
    await _sadutils.unsavegif(event, zzy)
    await sadevent.delete()
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
...
async def _(event):
    ...
    if not args:
        ...
        try:
            ...
            args = 2.0
    sadevent = await edit_or_reply(event, "__🎞Converting into Gif..__")
    ...
    zzy = await event.client.send_file(event.chat_id, result, reply_to=reply)
    await _sadutils.unsavegif(event, zzy)
    await sadevent.delete()
