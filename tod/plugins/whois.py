...
async def who(event):
    ...
    if not replied_user:
        return
    tod = await edit_or_reply(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(tod, "`Could not fetch info of that user.`")
    ...
    try:
        ...
        await tod.edit(caption, parse_mode="html")
