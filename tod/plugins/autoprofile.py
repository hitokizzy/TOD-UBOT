...
autopic_path = os.path.join(os.getcwd(), "tods", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "tods", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "tods", "photo_pfp.png")
...
async def autopicloop():
    ...
    while AUTOPICSTART:
        ...
        file = await tod.upload_file(autophoto_path)
        try:
            await tod(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await tod.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await tod(
                    functions.photos.DeletePhotosRequest(
                        await tod.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await tod(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        tod = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(tod, 200)
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await tod(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await tod(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
tod.loop.create_task(autopfp_start())
tod.loop.create_task(autopicloop())
tod.loop.create_task(digitalpicloop())
tod.loop.create_task(bloom_pfploop())
tod.loop.create_task(autoname_loop())
tod.loop.create_task(autobio_loop())
tod.loop.create_task(custompfploop())
