...
SADLOGO = "https://telegra.ph/file/486715ddbbb29da89b2ed.png"
...
@ibel.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@ibel.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@ibel.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@ibel.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
