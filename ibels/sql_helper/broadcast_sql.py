...
class sadBroadcast(BASE):
    __tablename__ = "sadbroadcast"
    ...
    def __repr__(self):
        return "<ibel Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, sadBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


sadBroadcast.__table__.create(checkfirst=True)

sadBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with sadBROADCAST_INSERTION_LOCK:
        broadcast_group = sadBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with sadBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(sadBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with sadBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(sadBroadcast.keywoard)
            .filter(sadBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(sadBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(sadBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(sadBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(sadBroadcast).all()
        for x in all_groups:
