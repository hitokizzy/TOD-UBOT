...
class sad_GlobalCollection(BASE):
    __tablename__ = "sad_globalcollection"
    ...
    def __repr__(self):
        return "<ibel Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, sad_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


sad_GlobalCollection.__table__.create(checkfirst=True)

sad_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with sad_GLOBALCOLLECTION:
        keyword_items = sad_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with sad_GLOBALCOLLECTION:
        keyword_items = SESSION.query(sad_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with sad_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(sad_GlobalCollection.keywoard)
            .filter(sad_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(sad_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(sad_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(sad_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(sad_GlobalCollection).all()
        for x in all_groups:
