...
class Gdrive(BASE):
    __tablename__ = "sadgdrive"
    ibel = Column(String(50), primary_key=True)

    def __init__(self, ibel):
        self.ibel = ibel
...
def is_folder(folder_id):
    try:
        return SESSION.query(Gdrive).filter(Gdrive.ibel == str(folder_id))
...
def rmparent_id(folder_id):
    note = SESSION.query(Gdrive).filter(Gdrive.ibel == folder_id)
    if note:
        note.delete()
        SESSION.commit()
