...
from ibels import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    ...
    try:
        await ibel.connect()
        config = await ibel(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == ibel.session.server_address:
                if ibel.session.dc_id != option.id:
                    LOGS.warning(
                        f"Fixed DC ID in session from {ibel.session.dc_id}"
                        f" to {option.id}"
                    )
                ibel.session.set_dc(option.id, option.ip_address, option.port)
                ibel.session.save()
                break
        bot_details = await ibel.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await ibel.start(bot_token=Config.TG_BOT_USERNAME)
        ibel.me = await ibel.get_me()
        ibel.uid = ibel.tgbot.uid = utils.get_peer_id(ibel.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(ibel.me)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await ibel.get_entity(BOTLOG_CHATID)
        ...
        _, groupid = await create_supergroup(
            "ibel-ubot BotLog Group", ibel, Config.TG_BOT_USERNAME, descript
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await ibel.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
                    LOGS.info(
