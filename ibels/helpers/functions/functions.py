...
async def age_verifisadion(event, reply_to_id):
    ALLOW_NSFW = gvarstatus("ALLOW_NSFW") or "False"
    if ALLOW_NSFW.lower() == "true":
        return False
    results = await event.client.inline_query(
        Config.TG_BOT_USERNAME, "age_verifisadion_alert"
    )
...
def higlighted_text(
    ...
    if font_name is None:
        font_name = "ibels/helpers/styles/impact.ttf"
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    ibel = await sticcers[c_lick].click("me", hide_via=True)
    if ibel:
        await borg.send_file(int(chat_id), ibel, reply_to=reply_to_id)
        await ibel.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    ibel = await sticcers[0].click("me", hide_via=True)
    if ibel:
        await bot.send_file(int(chat_id), ibel, reply_to=reply_to_id)
        await ibel.delete()
