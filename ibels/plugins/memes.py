...
# ibel-ubot module for having some fun with people.
import asyncio
import random
import re
...
from ..helpers import sadmemes
from ..helpers.utils import _sadutils, parse_pre
...
@ibel.sad_cmd(
    ...
    info={
        ...
        "examples": [
            "{tr}squirrelsay ibel-ubot",
            "{tr}milksay ibel-ubot",
            "{tr}ghostbustersghostbusterssay ibel-ubot",
        ],
    },
)
...
async def who(event):
    ...
    if replied_user is None:
        return
    caption = await sadmemes.slap(replied_user, event, mention)
    try:
...
async def decide(event):
    ...
    await event.delete()
    zzy = await event.client.send_message(
...
async def faces(owo):
    ...
    reply_text = re.sub(r"\!+", " " + random.choice(sadmemes.UWUS), reply_text)
    reply_text = reply_text.replace("ove", "uv")
    reply_text += " " + random.choice(sadmemes.UWUS)
...
async def gbun(event):
    ...
    sadevent = await edit_or_reply(event, "**Summoning out le Gungnir ❗️⚜️☠️**")
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1035034432:
            ...
            if len(gbunVar) > 0:
                ...
                jnl += no_reason
            await sadevent.edit(jnl)
    else:
        mention = "`Warning!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By Admin...\nReason: Potential spammer. `"
        await sadevent.edit(mention)
