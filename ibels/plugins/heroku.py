# Heroku manager for your ibel-ubot
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        ibel = await edit_or_reply(var, "`Getting information...`")
        ...
        ibel = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await ibel.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await ibel.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await ibel.edit(
        ...
        ibel = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await ibel.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await ibel.edit(f"`{variable}`**  does not exist**")

        await ibel.edit(f"`{variable}`  **successfully deleted**")
...
async def dyno_usage(dyno):
    ...
    headers = {
        ...
        "Accept": "applisadion/vnd.heroku+json; version=3.account-quotas",
    }
...
def prettyjson(obj, indent=2, maxlinelength=80):
    """Renders JSON content with indentation and line splits/consadenations to fit maxlinelength.
