...
async def sadgban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    sade = await edit_or_reply(event, "`gbanning.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, sade)
    if not user:
        return
    if user.id == ibel.uid:
        return await edit_delete(sade, "`why would I ban myself`")
    if gban_sql.is_gbanned(user.id):
        await sade.edit(
            f"`the `[user](tg://user?id={user.id})` is already in gbanned list any way checking again`"
        )
    else:
        gban_sql.sadgban(user.id, reason)
    san = await admin_groups(event.client)
    count = 0
    zzy = len(san)
    if zzy == 0:
        return await edit_delete(sade, "`you are not admin of atleast one group` ")
    await sade.edit(
    ...
    sadtaken = (end - start).seconds
    if reason:
        await sade.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {sadtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await sade.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gbanned in {count} groups in {sadtaken} seconds`!!"
...
async def sadgban(event):
    "To unban the person from every group where you are admin."
    sade = await edit_or_reply(event, "`ungbanning.....`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, sade)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        gban_sql.sadungban(user.id)
    else:
        return await edit_delete(
            sade, f"the [user](tg://user?id={user.id}) `is not in your gbanned list`"
        )
    san = await admin_groups(event.client)
    count = 0
    zzy = len(san)
    if zzy == 0:
        return await edit_delete(sade, "`you are not even admin of atleast one group `")
    await sade.edit(
    ...
    if reason:
        await sade.edit(
            f"[{user.first_name}](tg://user?id={user.id}`) was ungbanned in {count} groups in {sadtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await sade.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was ungbanned in {count} groups in {sadtaken} seconds`!!"
...
async def sadgkick(event):  # sourcery no-metrics
    "kicks the person in all groups where you are admin"
    sade = await edit_or_reply(event, "`gkicking.......`")
    start = datetime.now()
    user, reason = await get_user_from_event(event, sade)
    if not user:
        return
    if user.id == ibel.uid:
        return await edit_delete(sade, "`why would I kick myself`")
    san = await admin_groups(event.client)
    count = 0
    zzy = len(san)
    if zzy == 0:
    ...
    if reason:
        await sade.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {sadtaken} seconds`!!\n**Reason :** `{reason}`"
        )
    else:
        await sade.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {sadtaken} seconds`!!"
