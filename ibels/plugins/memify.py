# Made by @mrconfused and @hitokizzy
# memify plugin for ibel-ubot
import asyncio
import io
import os
import random
...
from ..helpers import asciiart, media_type, sad_meeme, sad_meme
...
async def maccmd(event):  # sourcery no-metrics
    ...
    sadevent = await event.edit("__Adding frame for media....__")
    args = event.pattern_match.group(1)
    force = bool(args)
    try:
        imag = await _sadtools.media_to_pic(sadevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
        ...
        return await edit_delete(sadevent, f"**Error in identifying image:**\n__{e}__")
    wid, hgt = image.size
...
async def memes(event):
    ...
    sadinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    if not reply:
    ...
    if gvarstatus("CNG_FONTS") is None:
        CNG_FONTS = "ibels/helpers/styles/impact.ttf"
    else:
        CNG_FONTS = gvarstatus("CNG_FONTS")
    if max(len(top), len(bottom)) < 21:
        await sad_meme(CNG_FONTS, top, bottom, meme_file, meme)
    else:
        await sad_meeme(top, bottom, CNG_FONTS, meme_file, meme)
    if cmd != "mmf":
        meme = convert_tosticker(meme)
    await event.client.send_file(
        event.chat_id, meme, reply_to=sadid, force_document=False
    )
...
async def lang(event):
    ...
    if input_str not in font_list:
        sadevent = await edit_or_reply(event, "`Give me a correct font name...`")
        await asyncio.sleep(1)
        await sadevent.edit(f"**Available Fonts names are here:-**\n\n{FONTS}")
    else:
        arg = f"ibels/helpers/styles/{input_str}"
...
async def memes(event):
    ...
    color1 = c_list[0]
    color2 = c_list[1]
    bgcolor = "#080808" if not sadinput else sadinput
    asciiart(meme_file, 0.3, 1.9, outputfile, color1, color2, bgcolor)
    await event.client.send_file(
        event.chat_id, outputfile, reply_to=sadid, force_document=False
    )
    await output[0].delete()
...
async def memes(event):
    ...
    await event.client.send_file(
        event.chat_id, outputfile, force_document=False, reply_to=sadid
    )
    await output[0].delete()
    for files in (outputfile, meme_file):
        if files and os.path.exists(files):
...
async def memes(event):
    ...
    sadinput = event.pattern_match.group(1)
    sadinput = 50 if not sadinput else int(sadinput)
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "`Reply to supported Media...`")
    san = "adeevaproject"
    ...
    try:
        await crop(meme_file, outputfile, sadinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not sadinput:
        sadinput = "50"
    if ";" in str(sadinput):
        sadinput, colr = sadinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, sadinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
