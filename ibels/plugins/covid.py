# corona virus stats for ibel-ubot
from covid import Covid

from . import covidindia, edit_delete, edit_or_reply, ibel
...
async def corona(event):
    ...
    sadevent = await edit_or_reply(event, "`Collecting data...`")
    ...
    if country_data:
        ...
        if data:
            sad1 = int(data["new_positive"]) - int(data["positive"])
            sad2 = int(data["new_death"]) - int(data["death"])
            sad3 = int(data["new_cured"]) - int(data["cured"])
            result = f"<b>Corona virus info of {data['state_name']}\
                ...
                \n🥺 New Cases   : <code>{sad1}</code>\
                \n😟 New Deaths : <code>{sad2}</code>\
                \n😃 New cured  : <code>{sad3}</code> </b>"
