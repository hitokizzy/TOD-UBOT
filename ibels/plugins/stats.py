...
#                           STRINGS                           #
# =========================================================== #
STAT_INDIsadION = "`Collecting stats, Wait man`"
...
async def stats(event):  # sourcery no-metrics
    ...
    ibel = await edit_or_reply(event, STAT_INDIsadION)
...
async def stats(event):  # sourcery no-metrics
    sadcmd = event.pattern_match.group(1)
    sadevent = await edit_or_reply(event, STAT_INDIsadION)
    start_time = time.time()
    ibel = "adeevaproject"
    hi = []
    hica = []
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await edit_delete(sadevent, "`unblock `@tgscanrobot` and then try`")
