...
async def _(event):
    ...
    if not replied_user:
        return
    sadevent = await edit_or_reply(event, "`Fetching userinfo wait....`")
...
""".format(
    ...
    await edit_or_reply(sadevent, caption)
...
async def who(event):
    ...
    if not replied_user:
        return
    ibel = await edit_or_reply(event, "`Fetching userinfo wait....`")
    replied_user = await event.client(GetFullUserRequest(replied_user.id))
    try:
        photo, caption = await fetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(ibel, "`Could not fetch info of that user.`")
    ...
    try:
        ...
        await ibel.edit(caption, parse_mode="html")
