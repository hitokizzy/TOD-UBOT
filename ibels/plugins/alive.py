...
from ibels import StartTime, ibel, sadversion
...
async def amireallyalive(event):
    ...
    if SAD_IMG:
        SAD = [x for x in SAD_IMG.split()]
        PIC = random.choice(SAD)
        try:
...
**{EMOJI} ibel-ubot Version :** `{sadver}`
...
async def amireallyalive(event):
    ...
    reply_to_id = await reply_id(event)
    EMOJI = gvarstatus("ALIVE_EMOJI") or ""
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**ibel-ubot is Up and Running**"
    sad_caption = f"{ALIVE_TEXT}\n"
    sad_caption += f"**{EMOJI} Telethon version :** `{version.__version__}\n`"
    sad_caption += f"**{EMOJI} ibel-ubot Version :** `{sadversion}`\n"
...
@ibel.tgbot.on(CallbackQuery(data=re.compile(b"stats")))
