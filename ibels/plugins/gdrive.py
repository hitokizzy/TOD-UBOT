...
from ibels.core.logger import logging
...
# ibel-ubot Google Drive managers  ported from Projectbish and added extra things by @mrconfused
...
async def download(event, gdrive, service, uri=None):  # sourcery no-metrics
    ...
    if uri:
        ...
        if sadtorrent:
            ...
            await edit_or_reply(
                gdrive,
                "`To use torrent files or download files from link install torrentutils from` @sadplugins",
            )
...
async def upload(gdrive, service, file_path, file_name, mimeType, dir_id=None):
    ...
    body = {
        "name": file_name,
        "description": "Uploaded from Telegram using ibel-ubot.",
        "mimeType": mimeType,
...
async def generate_credentials(gdrive):
    ...
    if not BOTLOG:
        await edit_delete(
            gdrive,
            "for authensadion you need to set PRIVATE_GROUP_BOT_API_ID in heroku",
            time=10,
        )
    hmm = gdrive.client.uid
    ...
    msg = await gdrive.respond(
        "`Go to your Private log group to authentisade token...`"
    )
...
async def g_download(event):
    ...
    file_name, sadprocess = await gdrive_download(event, sadevent, service, drive_link)
    if sadprocess is not None:
        return await edit_delete(sadevent, file_name)
    thumb = thumb_image_path if os.path.exists(thumb_image_path) else None
    if not cmd:
        await sadevent.edit("**File Downloaded.\nlocation : **`" + str(file_name) + "`")
    else:
        c_time = time.time()
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, sadevent, c_time, "Uploading...", file_name)
            ),
        )
        os.remove(file_name)
        await edit_delete(
...
async def gshare(event):
    ...
    sadevent = await edit_or_reply(event, "`Creating sharable link...`")
    await asyncio.sleep(2)
    await share(service, sadevent, input_str)
