...
@ibel.sad_cmd(
    ...
    info={
        "header": "To show system specifisadion.",
        "usage": "{tr}spc",
    },
)
async def psu(event):
    "shows system specifisadion"
...
async def cpu(event):
    ...
    cmd = "ibel /proc/cpuinfo | grep 'model name'"
    o = (await _sadutils.runcmd(cmd))[0]
    await edit_or_reply(
        event, f"**[ibel's](tg://need_update_for_some_feature/) CPU Model:**\n{o}"
    )
...
async def sysdetails(sysd):
    ...
    sadevent = await edit_or_reply(sysd, "`Fetching system information.`")
    cmd = "git clone https://github.com/dylanaraps/neofetch.git"
    await _sadutils.runcmd(cmd)
    neo = "neofetch/neofetch --off --color_blocks off --bold off --cpu_temp C \
                    --cpu_speed on --cpu_cores physical --kernel_shorthand off --stdout"
    a, b, c, d = await _sadutils.runcmd(neo)
    result = str(a) + str(b)
    await edit_or_reply(sadevent, "**Neofetch Result:** `" + result + "`")
