...
async def echo(event):
    ...
    sadevent = await edit_or_reply(event, "`Adding Echo to user...`")
    user, rank = await get_user_from_event(event, sadevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_or_reply(sadevent, "Hi")
...
@ibel.sad_cmd(incoming=True, edited=False)
