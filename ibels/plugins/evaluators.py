...
@ibel.sad_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec ibel stringsetup.py",
    },
)
async def _(event):
    ...
    sadevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    saduser = await event.client.get_me()
    curruser = saduser.username or "ibel-ubot"
    uid = os.geteuid()
    if uid == 0:
...
@ibel.sad_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('ibel-ubot')",
    },
)
async def _(event):
    ...
    sadevent = await edit_or_reply(event, "`Running ...`")
