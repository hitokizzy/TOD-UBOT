...
autopic_path = os.path.join(os.getcwd(), "ibels", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "ibels", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "ibels", "photo_pfp.png")
...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await ibel.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await ibel.upload_file(autophoto_path)
        try:
            await ibel(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await ibel.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await ibel(
                    functions.photos.DeletePhotosRequest(
                        await ibel.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await ibel(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        ibel = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(ibel, 200)
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await ibel(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await ibel(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await ibel(functions.photos.UploadProfilePhotoRequest(file))
        await _sadutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
ibel.loop.create_task(autopfp_start())
ibel.loop.create_task(autopicloop())
ibel.loop.create_task(digitalpicloop())
ibel.loop.create_task(bloom_pfploop())
ibel.loop.create_task(autoname_loop())
ibel.loop.create_task(autobio_loop())
ibel.loop.create_task(custompfploop())
