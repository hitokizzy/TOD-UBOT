...
from ibels import HEROKU_APP, UPSTREAM_REPO_URL, ibel
...
NO_HEROKU_APP_CFGD = "no heroku applisadion found, but a key given? 😕 "
HEROKU_GIT_REF_SPEC = "HEAD:refs/heads/master"
RESTARTING_APP = "re-starting heroku applisadion"
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"do `{cmdhd}update deploy` to update the ibel-ubot"
        )
...
async def upstream(event):
    ...
    off_repo = "https://github.com/hitokizzy/ibel-ubot"
    os.chdir("/bin")
    try:
        txt = "`Oops.. Updater cannot continue due to "
        txt += "some problems occured`\n\n**LOGTRACE:**\n"
...
@ibel.sad_cmd(
    pattern="ibel-ubot$",
    command=("ibel-ubot", plugin_category),
    info={
...
async def variable(var):
    ...
    heroku_var["UPSTREAM_REPO"] = "https://github.com/hitokizzy/ibel-ubot"
