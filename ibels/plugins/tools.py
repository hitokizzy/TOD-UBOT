...
from ..helpers.utils import _format, _sadutils, reply_id
...
async def currency(event):
    ...
    try:
        ...
        symbols = await AioHttp().get_raw(
            "https://raw.githubusercontent.com/hitokizzy/ibel-ubot-Resources/master/Resources/Data/currency.py"
        )
...
async def _(event):
    ...
    chat = "@VS_Robot"
    sadevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if response1.text:
                ...
                return await sadevent.edit(response1.text, parse_mode=_format.parse_pre)
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        ...
        if not input_str:
            return await edit_or_reply(sadevent, response4.text)
...
async def parseqr(event):
    ...
    sadevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _sadutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(sadevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(sadevent, f"**Failed to Decode:**\n`{result}`")
    except Exception as e:
        await edit_or_reply(sadevent, f"**Error:**\n`{e}`")
...
async def _(event):
    ...
    try:
        ...
        return await sadevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(sadevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    sadevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await sadevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await sadevent.edit(output_str, link_preview=True)
