...
async def sticklet(event):
    ...
    fontsize = 230
    FONT_FILE = await get_font_file(event.client, "@sadfonts", font_file_name)
    ...
    image_stream.name = "ibel-ubot.webp"
...
async def quby(event):
    ...
    if cmd == "b":
        ibel = convert_tosticker(file_name)
        await event.client.send_file(
            event.chat_id, ibel, reply_to=reply_to_id, force_document=False
        )
    else:
