# reverse search and google search  plugin for ibel
import io
import os
import re
import urllib
...
@ibel.sad_cmd(
    ...
    info={
        ...
        "examples": [
            ...
            "{tr}gs -l6 ibel-ubot",
            "{tr}gs -p2 ibel-ubot",
            "{tr}gs -p2 -l7 ibel-ubot",
        ],
    },
)
async def gsearch(q_event):
    "Google search command."
    sadevent = await edit_or_reply(q_event, "`searching........`")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        sadevent = await edit_or_reply(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("location")
        await sadevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                sadevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        sadevent = event
    await edit_or_reply(sadevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
...
async def _(img):
    ...
    if message and message.media:
        ...
        return
    if photo:
        sadevent = await edit_or_reply(img, "`Processing...`")
        try:
            image = Image.open(photo)
        except OSError:
            return await sadevent.edit("`Unsupported , most likely.`")
        name = "okgoogle.png"
        image.save(name, "PNG")
        image.close()
        ...
        if response != 400:
            ...
            return await sadevent.edit("`Unable to perform reverse search.`")
        ...
        if guess and imgspage:
            await sadevent.edit(f"[{guess}]({fetchUrl})\n\n`Looking for this Image...`")
        else:
            return await sadevent.edit("`Can't find any kind similar images.`")
