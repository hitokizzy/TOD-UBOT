...
async def imirror(event):  # sourcery no-metrics
    ...
    sadevent = await event.edit("__Reflecting the image....__")
    args = event.pattern_match.group(1)
    if args:
        filename = "ibel-ubot.webp"
        f_format = "webp"
    else:
        filename = "ibel-ubot.jpg"
        f_format = "jpeg"
    try:
...
async def irotate(event):
    ...
    args = event.pattern_match.group(1)
    sadevent = await edit_or_reply(event, "__Rotating the replied media...__")
    imag = await _sadtools.media_to_pic(sadevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
...
async def iresize(event):
    ...
    sadevent = await edit_or_reply(event, "__Resizeing the replied media...__")
    imag = await _sadtools.media_to_pic(sadevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
    ...
    if len(args) == 1:
        try:
            ...
            return await edit_delete(sadevent, "**Error:**\n__Invalid dimension.__")
    else:
        try:
            nw = int(args[0])
        except ValueError:
    ...
    try:
        ...
        return await edit_delete(sadevent, f"**Error:** __While resizing.\n{e}__")
    await event.delete()
    img = io.BytesIO()
    img.name = "ibel-ubot.png"
    image.save(img, "PNG")
...
async def square_cmd(event):
    ...
    sadevent = await event.edit("__Adding borders to make it square....__")
    try:
        imag = await _sadtools.media_to_pic(sadevent, reply, noedits=True)
        if imag[1] is None:
            return await edit_delete(
...
async def pic_gifcmd(event):
    ...
    if args:
        ...
        pix = 100
    sadevent = await edit_or_reply(event, "__🎞Dotifying image...__")
    imag = await _sadtools.media_to_pic(sadevent, reply, noedits=True)
    if imag[1] is None:
        return await edit_delete(
            imag[0], "__Unable to extract image from the replied message.__"
