# by  @hitokizzy ( https://t.me/mrconfused  )

# songs finder for ibel-ubot
import asyncio
import io
import os
...
async def _(event):
    ...
    ibel = "adeevaproject"
    sadevent = await edit_or_reply(event, "`wi8..! I am finding your song....`")
    ...
    try:
        ibel = Get(ibel)
        await event.client(ibel)
    except BaseException:
        pass
    stderr = (await _sadutils.runcmd(song_cmd))[1]
    if stderr:
        return await sadevent.edit(f"**Error :** `{stderr}`")
    sadname, stderr = (await _sadutils.runcmd(name_cmd))[:2]
    if stderr:
        return await sadevent.edit(f"**Error :** `{stderr}`")
    # stderr = (await runcmd(thumb_cmd))[1]
    sadname = os.path.splitext(sadname)[0]
    # if stderr:
    #    return await sadevent.edit(f"**Error :** `{stderr}`")
    song_file = Path(f"{sadname}.mp3")
    if not os.path.exists(song_file):
        return await sadevent.edit(
            f"Sorry!. I can't find any related video/audio for `{query}`"
        )
    await sadevent.edit("`yeah..! i found something wi8..🥰`")
    sadthumb = Path(f"{sadname}.jpg")
    if not os.path.exists(sadthumb):
    ...
    for files in (sadthumb, song_file):
...
async def _(event):
    ...
    stderr = (await _sadutils.runcmd(video_cmd))[1]
    if stderr:
        return await sadevent.edit(f"**Error :** `{stderr}`")
    sadname, stderr = (await _sadutils.runcmd(name_cmd))[:2]
    if stderr:
    ...
    #    return await sadevent.edit(f"**Error :** `{stderr}`")
    sadname = os.path.splitext(sadname)[0]
    vsong_file = Path(f"{sadname}.mp4")
    if not os.path.exists(vsong_file):
        vsong_file = Path(f"{sadname}.mkv")
    ...
    await sadevent.delete()
    for files in (sadthumb, vsong_file):
        if files and os.path.exists(files):
            os.remove(files)
...
async def shazamcmd(event):
    ...
    sadevent = await edit_or_reply(event, "__Downloading the audio clip...__")
    try:
        ...
        LOGS.error(e)
        return await edit_delete(
            sadevent, f"**Error while reverse searching song:**\n__{e}__"
        )
...
async def _(event):
    ...
    sadevent = await edit_or_reply(event, SONG_SEARCH_STRING, parse_mode="html")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if baka[0].message.startswith(
                ...
                return await edit_delete(
                    sadevent, SONG_NOT_FOUND, parse_mode="html", time=5
                )
            await sadevent.edit(SONG_SENDING_STRING, parse_mode="html")
            ...
            return await sadevent.edit(SONGBOT_BLOCKED_STRING, parse_mode="html")
        await event.client.send_file(
            event.chat_id,
            music,
...
async def _(event):
    ...
    chat = "@auddbot"
    sadevent = await edit_or_reply(event, "```Identifying the song```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await sadevent.edit("Wait just a sec...")
            result = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await sadevent.edit("```Please unblock (@auddbot) and try again```")
            return
    namem = f"**Song Name : **`{result.text.splitlines()[0]}`\
        \n\n**Details : **__{result.text.splitlines()[2]}__"
    await sadevent.edit(namem)
