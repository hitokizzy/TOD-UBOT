...
from ..helpers.utils import _sadutils
...
async def spam_function(event, zzy, ibel, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(ibel[0])
    if len(ibel) == 2:
        spam_message = str(ibel[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await zzy.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and zzy.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            zzy = await event.client.send_file(event.chat_id, zzy, caption=zzy.text)
        ...
        if BOTLOG:
            ...
            zzy = await event.client.send_file(BOTLOG_CHATID, zzy)
            await _sadutils.unsavegif(event, zzy)
        return
    elif event.reply_to_msg_id and zzy.text:
        spam_message = zzy.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
async def spammer(event):
    ...
    zzy = await event.get_reply_message()
    ...
    await spam_function(event, zzy, ibel, sleeptimem, sleeptimet)
...
@ibel.sad_cmd(
    ...
    info={
        ...
        "usage": "{tr}wspam <text>",
        "examples": "{tr}wspam I am using ibel-ubot",
    },
)
async def tmeme(event):
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, ibel, sleeptimem, sleeptimet, DelaySpam=True)
