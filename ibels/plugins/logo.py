...
async def bad(event):
    ...
    for i, each in enumerate(links, start=1):
        ibel = os.path.splitext(each.text)[0]
        bg_name.append(ibel)
        lbg_list += f"**{i}.**  `{ibel}`\n"
    ...
    if cmd == "c":
        ...
        if not input_str and event.reply_to_msg_id and reply_message.media:
            if not os.path.isdir("./temp"):
                os.mkdir("./temp")
            output = await _sadtools.media_to_pic(event, reply_message)
    ...
    if input_str not in bg_name:
        sadevent = await edit_or_reply(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await edit_delete(sadevent, lbg_list, time=60)
    else:
...
async def pussy(event):
    ...
    if cmd == "":
        ...
        if input_str not in logo_font:
            ...
            await asyncio.sleep(1)
            await edit_delete(sadevent, font_name, time=80)
        else:
            if " " in input_str:
        ...
        if input_str not in fg_name:
            sadevent = await edit_or_reply(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await edit_delete(
                sadevent,
                f"**Available color names are here:-**\n\n{fg_list}",
        ...
        ibel = re.compile(r"^\-?[1-9][0-9]*\.?[0-9]*")
        isint = re.match(ibel, input_str)
