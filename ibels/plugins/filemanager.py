...
from ..helpers.utils import _format, _sadutils
...
async def lst(event):  # sourcery no-metrics
    ...
    ibel = "".join(event.text.split(maxsplit=1)[1:])
    path = ibel or os.getcwd()
    if not os.path.exists(path):
        await edit_or_reply(
            event,
            f"there is no such directory or file with the name `{ibel}` check again",
        )
        return
    path = Path(ibel) if ibel else os.getcwd()
    if os.path.isdir(path):
        ...
        for contents in sorted(lists):
            sadpath = os.path.join(path, contents)
            if not os.path.isdir(sadpath):
                size = os.stat(sadpath).st_size
...
async def lst(event):
    ...
    ibel = event.pattern_match.group(1)
    if ibel:
        path = Path(ibel)
    else:
...
async def _(event):
    ...
    try:
        await _sadutils.runcmd(f"mkdir {original}")
...
async def _(event):
    ...
    await asyncio.sleep(2)
    try:
        await _sadutils.runcmd(f"cp -r {original} {location}")
