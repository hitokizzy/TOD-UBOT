...
async def group_has_sedbot(group):
    if isinstance(group, types.InputPeerChannel):
        full = await ibel(functions.channels.GetFullChannelRequest(group))
    elif isinstance(group, types.InputPeerChat):
        full = await ibel(functions.messages.GetFullChatRequest(group.chat_id))
    else:
        return False
...
@ibel.on(events.NewMessage)
async def on_message(event):
    last_msgs[event.chat_id].appendleft(event.message)


@ibel.on(events.MessageEdited)
