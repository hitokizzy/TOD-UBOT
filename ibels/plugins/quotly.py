...
@ibel.sad_cmd(
    ...
    info={
        ...
        "examples": ["{tr}qpic ibel-ubot.", "{tr}qpic -b ibel-ubot."],
    },
)
async def q_pic(event):  # sourcery no-metrics
    ...
    sadevent = await edit_or_reply(event, "__Making Quote pic....__")
    ...
    if (
        ...
        or (
            mediatype == "Sticker"
            and reply.document.mime_type == "applisadion/i-tgsticker"
        )
    ):
    ...
    output = io.BytesIO()
    if sticker:
        output.name = "ibel-ubot.Webp"
        img.save(output, "webp")
    else:
        output.name = "ibel-ubot.png"
        img.save(output, "PNG")
    output.seek(0)
...
async def _(event):
    ...
    chat = "@QuotLyBot"
    sadevent = await edit_or_reply(event, "```Making a Quote```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            if messages_id != []:
                ...
                return await edit_delete(
                    sadevent, "`I guess you have used a invalid syntax`"
                )
