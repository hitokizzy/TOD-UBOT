...
# baning spmmers plugin for ibel-ubot by @hitokizzy
...
from . import BOTLOG, BOTLOG_CHATID, edit_or_reply, logging, ibel, spamwatch
...
if Config.ANTISPAMBOT_BAN:
    ...
    async def anti_spambot(event):  # sourcery no-metrics
        ...
        sadadmin = await is_admin(event.client, event.chat_id, event.client.uid)
        if not sadadmin:
            return
        sadbanned = None
        adder = None
        ...
        if is_gbanned(user.id):
            sadgban = get_gbanuser(user.id)
            if sadgban.reason:
                hmm = await event.reply(
                    f"[{user.first_name}](tg://user?id={user.id}) was gbanned by you for the reason `{sadgban.reason}`"
                )
            else:
                hmm = await event.reply(
            ...
            try:
                ...
                LOGS.info(e)
        if spamwatch and not sadbanned:
...
async def caschecker(event):
    ...
    try:
        ...
        await sadevent.edit("`CAS check failed: Admin privileges are required`")
        return
    except BaseException:
        await sadevent.edit("`CAS check failed`")
        return
    await sadevent.edit(text)
...
async def caschecker(event):
    ...
    try:
        ...
        await sadevent.edit("`spamwatch check failed: Admin privileges are required`")
        return
    except BaseException:
        await sadevent.edit("`spamwatch check failed`")
        return
    await sadevent.edit(text)
