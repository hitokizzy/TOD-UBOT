...
async def tmuter(event):  # sourcery no-metrics
    ...
    sadevent = await edit_or_reply(event, "`muting....`")
    user, reason = await get_user_from_event(event, sadevent)
    if not user:
        return
    if not reason:
        return await sadevent.edit("you haven't mentioned time, check `.help tmute`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    sadtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ctime = await extract_time(sadevent, sadtime)
    if not ctime:
        return
    if user.id == event.client.uid:
        return await sadevent.edit("Sorry, I can't mute myself")
    try:
        await sadevent.client(
            EditBannedRequest(
                event.chat_id,
                user.id,
...
async def tban(event):  # sourcery no-metrics
    ...
    sadevent = await edit_or_reply(event, "`banning....`")
    user, reason = await get_user_from_event(event, sadevent)
    if not user:
        return
    if not reason:
        return await sadevent.edit("you haven't mentioned time, check `.help tban`")
    reason = reason.split(" ", 1)
    hmm = len(reason)
    sadtime = reason[0].strip()
    reason = "".join(reason[1:]) if hmm > 1 else None
    ...
    if not ctime:
        return
    if user.id == event.client.uid:
        return await sadevent.edit("Sorry, I can't ban myself")
    await sadevent.edit("`Whacking the pest!`")
    try:
        await event.client(
            EditBannedRequest(
                event.chat_id,
