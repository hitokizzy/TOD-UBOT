...
async def grpinfo():
    outstr = "**Plugins in ibel-ubot are:**\n\n"
...
async def cmdlist():
    outstr = "**Total list of Commands in your ibel-ubot are :**\n\n"
    category = ["admin", "bot", "fun", "misc", "tools", "utils", "extra"]
    for ibel in category:
        plugins = GRP_INFO[ibel]
        outstr += f"**{emoji[ibel]} {ibel.title()} ** - {len(plugins)}\n\n"
...
@ibel.sad_cmd(
    ...
    info={
        "header": "To get guide for ibel-ubot.",
...
async def _(event):
    ...
    if not input_str:
        ...
        outstr += f"**Usage : ** `{cmdprefix}help -c <command name>`"
    await edit_or_reply(
        event, outstr, aslink=True, linktext="Total Commands of ibel-ubot are :"
    )
...
async def _(event):
    ...
    if found:
        ...
        out = f"I can't find any such command `{cmd}` in ibel-ubot"
